...
export default function EditBrandModal({ open, onOpenChange, brand }) {
  ...
  const [editMainCategoryModalOpen, setEditMainCategoryModalOpen] = useState(false);
  ...
  const mainCategoriesData = allMainCategoriesData?.filter(category => 
  ...
  const subCategoriesData = allSubCategoriesData?.filter(subCategory => {
    ...
    return mainCategoriesData?.some(mainCategory => 
  ...
  function updateBrand(values) {
    ...
    if (values.background_image_url && typeof values.background_image_url !== 'string') {
              ...
              <div className="space-y-4">
                ...
                <div className="w-full flex flex-col md:flex-row gap-4 items-center text-[#1243AF]">
                  ...
                  <div className="flex flex-col w-full md:w-1/2 text-left md:text-right">
                    <label htmlFor="edit_short_description_ar" className="text-sm">
                      الوصف المختصر عربى
                    </label>
                    <textarea
                      maxLength={80}
                ...
                <div className="w-full flex flex-col md:flex-row gap-4 items-center text-[#1243AF]">
                  ...
                  <div className="flex flex-col w-full md:w-1/2 text-left md:text-right">
                    <label htmlFor="edit_full_description_ar" className="text-sm">
                      الوصف عربى
                    </label>
                    <textarea
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {mainCategoriesData.map((category) => (
                      <div key={category.id} className="bg-gray-50 p-4 rounded-lg textColor border flex justify-between items-center">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h3 className="font-semibold text-[#1243AF]">{category.name_en}</h3>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg text-center">
                    <p className="text-sm text-gray-500">
                      No main categories found for <strong>{brand?.name_en}</strong>.
                    </p>
                ...
                {subCategoriesData && subCategoriesData.length > 0 ? (
                  <div className="space-y-3">
                    {subCategoriesData.map((subCategory) => {
                      ...
                      const mainCategory = mainCategoriesData?.find(mc => mc.id === subCategory.main_category_id);
                      return (
                        <div key={subCategory.id} className="bg-gray-50 p-4 rounded-lg border flex textColor justify-between items-center">
                          <div className="flex items-center gap-4">
                            <div>
                              <h3 className="font-semibold text-[#1243AF]">{subCategory.name_en}</h3>
                              <p className="text-sm text-gray-600">{subCategory.name_ar}</p>
                              <p className="text-xs text-gray-500">
                               Main Category: {mainCategory?.name_en || 'Unknown'}
                              </p>
                            </div>
                          </div>
                          <button
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg text-center">
                    <p className="text-sm text-gray-500">
                      No sub categories found for <strong>{brand?.name_en}</strong>.
                    </p>
                    <p className="text-xs text-gray-400 mt-1">
                      Add main categories first, then sub categories will appear here.
                    </p>
                  </div>
                )}
              </div>
